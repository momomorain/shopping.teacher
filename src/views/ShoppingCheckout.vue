...
<template>
  <div class="w-full px-5">
    ...
    <section class="mb-6 px-5">
      ...
      <div>
        <div v-for="item in checkData " :key="item.id" class="grid grid-cols-4 py-1 border-b min-h-[80px] gap-x-3 md:gap-x-1 lg:first:border-t">
    ...
    <form @submit.prevent="submitCart">
      ...
      <div class="mb-6 px-5">
        ...
        <div class="flex gap-5">
          <label class="flex items-center cursor-pointer">
            <input v-model="buyerData.paymentMethod" type="radio" id="payment-method1" value="信用卡" class="mr-2 cursor-pointer" required name="paymentMethod"/>
            <span>信用卡</span>
          </label>

          <label class="flex items-center cursor-pointer">
            <input v-model="buyerData.paymentMethod" type="radio" id="payment-method2" value="貨到付款" class="mr-2 cursor-pointer" required name="paymentMethod"/>
            <span>貨到付款</span>
          </label>
        </div>
      </div>
      ...
      <div class="mb-6 px-5">
        ...
        <div class="flex gap-5">
          <label class="flex items-center cursor-pointer">
            <input v-model="buyerData.deliver" type="radio" id="transport-method1" value="宅配" required />
            <span>宅配</span>
          </label>
          <label class="flex items-center cursor-pointer">
            <input v-model="buyerData.deliver" type="radio" id="transport-method2" value="超商取貨" class="mr-2 cursor-pointer" required />
            <span>超商取貨</span>
          </label>
        </div>
      </div>
